...
## Testing the thermal camera connection

This branch adds tooling to exercise Protogon's Qwiic/I²C path with an MLX90640 thermal camera. Both tools ship a host simulator, so you can try them without a badge.

* [`app/`](app/) — a badge app that shows the camera as a live heatmap. It installs onto the EEPROM (`app/provision_protogon.py`, one `mpremote` command) so the badge auto-launches it on insert. See [`app/README.md`](app/README.md).
* [`diagnostics/`](diagnostics/) — an I²C bus stress-test and camera bring-up tool: it hammers the bus at 133 kHz and reports bit errors. See [`diagnostics/README.md`](diagnostics/README.md).
